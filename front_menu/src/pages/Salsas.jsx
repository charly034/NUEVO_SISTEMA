...
import { useDeleteSalsa, useSalsas, useUpdateSalsa } from '../hooks/useSalsas.js';
import SalsasPanel from '../components/platos/SalsasPanel.jsx';
...
function hayFiltroActivo({ search, activoFilter }) {
  return Boolean(search || activoFilter !== 'true');
}

function textoEstadoSalsas({ search, activoFilter, totalBase }) {
  if (totalBase === 0) {
    return {
      titulo: 'No hay salsas cargadas aun.',
      detalle: 'Cuando crees la primera salsa, va a aparecer en este listado.',
    };
  }
  if (search) {
    return {
      titulo: `No se encontraron salsas para "${search}".`,
      detalle: 'Probá con otro nombre o limpiá los filtros activos.',
    };
  }
  if (activoFilter === 'false') {
    return {
      titulo: 'No hay salsas inactivas.',
      detalle: 'Todas las salsas visibles estan activas.',
    };
  }
  return {
    titulo: 'No se encontraron salsas para este filtro.',
...
function SalsaMobileCard({ salsa, loading, onOpen, onToggleActivo }) {
  return (
    <div className="bg-white px-4 py-3.5">
      <button type="button" onClick={onOpen} className="flex w-full items-start gap-3 text-left">
        <div className="min-w-0 flex-1">
          <p className="font-semibold leading-snug text-gray-900">{salsa.nombre}</p>
        </div>
      </button>
...
export default function Salsas() {
  ...
  const { data: salsas = [], isLoading } = useSalsas();
  const actualizar = useUpdateSalsa();
  const eliminar = useDeleteSalsa();
  ...
  const filtradas = useMemo(() => {
    let list = salsas.filter((s) => {
      const coincideTexto = !search || s.nombre.toLowerCase().includes(search.toLowerCase());
      const coincideEstado =
        activoFilter === undefined ||
        (activoFilter === 'true' ? s.activo : !s.activo);
    ...
    return list;
  }, [activoFilter, salsas, search, sortBy, sortDir]);

  const totalPages = Math.max(1, Math.ceil(filtradas.length / PAGE_SIZE));
  const safePage = Math.min(page, totalPages);
  const paginadas = filtradas.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
  const filtrosActivos = hayFiltroActivo({ search, activoFilter });
  const emptyState = textoEstadoSalsas({
    search,
    activoFilter,
    totalBase: salsas.length,
  });
  ...
  const startEdit = (salsa) => {
    setEditError('');
    setDetalle(null);
    setEditando({ id: salsa.id, nombre: salsa.nombre });
  };
  const limpiarFiltros = () => setSearchParams(new URLSearchParams(), { replace: true });

  const handleToggleActivo = async (salsa) => {
    try {
      await actualizar.mutateAsync({ id: salsa.id, data: { activo: !salsa.activo } });
      toast.success(salsa.activo ? 'Salsa desactivada' : 'Salsa activada');
    } catch (err) {
      toast.error(err?.message || 'Error');
    }
  };

  const handleDelete = async (salsa) => {
    if (!await confirmar({ titulo: `Eliminar "${salsa.nombre}"?`, texto: 'Esta acción no se puede deshacer.', botonConfirmar: 'Sí, eliminar' })) return;
    try {
      await eliminar.mutateAsync(salsa.id);
  ...
  const handleGuardarEdit = async (e) => {
    ...
    try {
      await actualizar.mutateAsync({
        id: editando.id,
        data: { nombre: editando.nombre.trim() },
      });
      setEditando(null);
      setEditError('');
      toast.success('Salsa actualizada');
    } catch (err) {
      toast.error(err?.message || 'Error');
    }
  };
    ...
    <div className="mx-auto max-w-7xl space-y-4 p-4 md:space-y-5 md:p-6">
      <div className="flex items-start justify-between gap-3">
        <div className="min-h-[52px]">
          <h1 className="text-2xl font-bold text-gray-900">Salsas</h1>
          <p className="mt-0.5 h-5 text-sm text-gray-500">
            {salsas.length} salsa{salsas.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button type="button" onClick={() => setModalCreate(true)} className="btn-primary flex-shrink-0">+ Nueva salsa</button>
      </div>
              ...
              <table className="hidden w-full text-sm md:table">
                ...
                <tbody className="divide-y divide-gray-50">
                  {paginadas.length === 0 ? (
                    ...
                    <tr key={s.id} onClick={() => setDetalle(s)} className="group cursor-pointer transition-colors hover:bg-gray-50/80">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          <IconActionButton label={`Ver detalle de ${s.nombre}`} tooltip="Ver detalle" onClick={() => setDetalle(s)}>
                            <PencilIcon />
                          </IconActionButton>
                        </div>
                      </td>
      ...
      <SalsasPanel modalOpen={modalCreate} onModalClose={() => setModalCreate(false)} />

      <SideDrawer open={!!detalle} onClose={() => setDetalle(null)} title="Detalle de salsa" width="md">
        {detalle ? (
          <div className="flex h-full flex-col p-5">
            <div className="flex-1 space-y-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Salsa</p>
              ...
              >
                {detalle.activo ? 'Desactivar salsa' : 'Activar salsa'}
              </button>
            </div>
